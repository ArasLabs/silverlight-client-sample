...
public sealed class MD5Core
{
        ...
        for (int i = 0; i <= chunks; i++)
        {
            ...
            if (i == chunks)
            {
                ...
                if (0 <= remainder)
                {
                    ...
                    Array.Copy(input, chunks * 64, working, 0, remainder);
            ...
            {
                Array.Copy(input, working, 64+remainder);
    ...
    private static uint[] Converter(byte[] input)
    {
        ...
        if(input.Length != 64)
            throw new System.ArgumentException("Input length must be 64 bytes", "input");

        uint[] result = new uint[16];

        for (int i = 0; i < 16; i++)
        {
            result[i] = (uint)input[i * 4];
